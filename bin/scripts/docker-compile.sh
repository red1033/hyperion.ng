...
REGISTRY_URL="ghcr.io/hyperion-project/hyperion.ng"
# cmake build type
BUILD_TYPE="Release"
# the docker image tag at GitHub Container Registry
BUILD_TARGET="x86_64"
...
## Without arguments it will compile Hyperion for Debain Stretch (x86_64) or higher.
## Supports Raspberry Pi (armv6l, armv7l) cross compilation (Debian Stretch) and native compilation (Raspbian Stretch/Buster)
##
...
# docker-compile.sh -t x86_64       # The docker tag, one of x86_64 | armv6l | armv7l | rpi-raspbian-stretch | rpi-raspbian-buster
...
echo "---> Startup docker..."
$DOCKER pull $REGISTRY_URL:$BUILD_TARGET
$DOCKER run --rm \
	-v "${SCRIPT_PATH}/deploy:/deploy" \
	-v "${SCRIPT_PATH}/hyperion:/source:ro" \
	$REGISTRY_URL:$BUILD_TARGET \
